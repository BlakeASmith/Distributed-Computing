...
type Stats struct {
	...
	// (in-memory) space required to store intermediates
...
func main() {
	...
	stat.NumKeyBytes = keysum
	stat.NumValBytes = valsum
	stat.IntermediateSpace = keysum + valsum
	stat.NumRecords = len(intermediate)
	...
	stat.ReduceSequenceTime = elapsed
	done := time.Since(start)
	stat.TotalTime = done
	stat.NumKeys = reducecalls
